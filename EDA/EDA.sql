...
ALTER TABLE ad_staging
ADD COLUMN Movement_Numeric INT;

UPDATE ad_staging
SET Movement_Numeric = 
    CASE 
        WHEN User_Movement_Data = 'no movement' THEN 1
		WHEN User_Movement_Data = 'movement only' THEN 2
        WHEN User_Movement_Data = 'gaze, movement' THEN 3
    END
WHERE Ad_ID > 0;
...
WITH CategoryCounts AS (
    ...
    UNION ALL
    SELECT 'User_Movement_Data', User_Movement_Data, COUNT(*)
    FROM ad_staging
    GROUP BY User_Movement_Data
)
SELECT 
    Category,
    Value,
